<script>
  ...
  export let url = 'https://e54a8car3bcq7q8h.public.blob.vercel-storage.com/14c24a03-fe3c-46e9-9c60-b3629b7bd407-AJU9Re3SULCQPsu1txSmrreuXcjdy9.mp4';
  ...
  function setupMp4Box() {
    ...
    mp4boxfile.onReady = function (info) {
      ...
      initSegs.forEach((seg) => {
        const codec = seg.codec || info.tracks.find((t) => t.id === seg.id)?.codec;
        if (!codec) {
    ...
    mp4boxfile.onSegment = function (id, user, buffer, sampleNum, is_last) {
      console.log(`Received segment for track ${id}, samples up to #${sampleNum}`);
      if (sourceBuffers[id] && !sourceBuffers[id].updating) {
        ...
        console.error(`Pending segments queue not initialized for track ID: ${id}`);
      }
    };
  }

  function onUpdateEnd(trackId) {
    if (pendingSegments[trackId]?.length > 0 && !sourceBuffers[trackId].updating) {
